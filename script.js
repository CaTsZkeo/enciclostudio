...
(() => {
    ...
    const createLocalArticleId = existing => { let id; do { id = `local-${globalThis.crypto?.randomUUID?.() || `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`}`; } while (existing.some(article => article.id === id)); return id; };
    const normalizeLocalResource = value => { const resource = String(value || "").trim(); if (!resource) return ""; if (/^https?:\/\//i.test(resource) || /^(?:\.{0,2}\/)?[\w./%-]+(?:\?[^\s]*)?(?:#[^\s]*)?$/i.test(resource)) return resource; return ""; };
    const createLocalArticle = (values, existing) => { const now = new Date().toISOString(); return { id: createLocalArticleId(existing), title: String(values.title || "").trim(), subject: String(values.subject || "").trim(), level: String(values.level || "").trim(), summary: String(values.summary || "").trim(), resource: normalizeLocalResource(values.resource), date: now, updatedAt: now, status: "local", contentType: "article" }; };
    document.getElementById("articleEditor")?.addEventListener("submit", event => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); const custom = savedArticles(); custom.unshift(createLocalArticle(values, custom)); localStorage.setItem("enciclostudio-articles", JSON.stringify(custom)); document.getElementById("editorStatus").textContent = "Artículo añadido. Ya aparece en la biblioteca de inicio de este navegador."; event.currentTarget.reset(); });
})();
